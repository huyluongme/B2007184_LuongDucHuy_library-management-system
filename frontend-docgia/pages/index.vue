  ...
  <div>
    <VForm class="login-form" @submit="login">
      ...
      <VCard class="mx-auto pa-12 pb-8" elevation="8" max-width="448" rounded="lg">
        ...
        <VTextField density="compact" placeholder="Nhập mã độc giả" prepend-inner-icon="mdi-email-outline"
          variant="outlined" v-model="MSNV"></VTextField>
        ...
        <v-card-text class="text-center">
          <a class="text-blue text-decoration-none" href="/registers" rel="noopener noreferrer" target="_blank">
...
async function login(event: Event) {
  ...
  console.log("Fuck")
  try {
    const res = await authService.login(MSNV.value, MATKHAU.value, 'log')
